...
import dev.disaverio.wlkconverter.models.data.DayData
import dev.disaverio.wlkconverter.models.data.MonthData
import dev.disaverio.wlkconverter.models.wlk.DailySummary1
import dev.disaverio.wlkconverter.models.wlk.DailySummary2
import dev.disaverio.wlkconverter.models.wlk.HeaderBlock
import dev.disaverio.wlkconverter.models.wlk.MonthData as WlkMonthData
import dev.disaverio.wlkconverter.models.wlk.DayData as WlkDayData
import dev.disaverio.wlkconverter.models.wlk.WeatherDataRecord as WlkWeatherDataRecord
import dev.disaverio.wlkconverter.types.WindDirection
...
class WlkTransformerTest {
    ...
    private fun assertWeatherDataRecordAreEqual(wlkDataRecord: WlkWeatherDataRecord, dataRecord: WeatherDataRecord) {
        ...
        assertEquals(WlkFieldsTranslator.getPressure(wlkDataRecord.barometer)?.inHg, dataRecord.barometer?.inHg)
        ...
        assertEquals(WlkFieldsTranslator.getPrecipitationFromClicksQuantity(wlkDataRecord.rain).mm, dataRecord.rain.mm)
        assertEquals(WlkFieldsTranslator.getRainRateFromClicksNumber(wlkDataRecord.rain, wlkDataRecord.hiRainRate).mmph, dataRecord.hiRainRate.mmph)
        assertEquals(WlkFieldsTranslator.getSpeed(wlkDataRecord.windSpeed)?.milesph, dataRecord.windSpeed?.milesph)
        assertEquals(WlkFieldsTranslator.getSpeed(wlkDataRecord.hiWindSpeed)?.milesph, dataRecord.hiWindSpeed?.milesph)
    ...
    private fun assertDailySummaryAreEqual(ds1: DailySummary1, ds2: DailySummary2, ds: DailySummary) {
        ...
        assertEquals(WlkFieldsTranslator.getPressure(ds1.hiBar)?.inHg, ds.hiBar?.inHg)
        assertEquals(WlkFieldsTranslator.getPressure(ds1.lowBar)?.inHg, ds.lowBar?.inHg)
        assertEquals(WlkFieldsTranslator.getPressure(ds1.avgBar)?.inHg, ds.avgBar?.inHg)
        assertEquals(WlkFieldsTranslator.getSpeed(ds1.hiSpeed)?.milesph, ds.hiSpeed?.milesph)
        assertEquals(WlkFieldsTranslator.getSpeed(ds1.avgSpeed)?.milesph, ds.avgSpeed?.milesph)
        assertEquals(WlkFieldsTranslator.getWindRun(ds1.dailyWindRunTotal)?.miles, ds.dailyWindRunTotal?.miles)
        assertEquals(WlkFieldsTranslator.getSpeed(ds1.hi10MinSpeed)?.milesph, ds.hi10MinSpeed?.milesph)
        ...
        assertEquals(WlkFieldsTranslator.getPrecipitation(ds1.dailyRainTotal).inches, ds.dailyRainTotal.inches)
        assertEquals(WlkFieldsTranslator.getRainRate(ds1.hiRainRate).inchesph, ds.hiRainRate.inchesph)
